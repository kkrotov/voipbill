...
protected:
    ...
    string sql(BDb * db) {
        ...
        return "   select id, name, trunk_name_alias, code, source_rule_default_allowed, destination_rule_default_allowed, default_priority, auto_routing, route_table_id, our_trunk, auth_by_number, \
                orig_redirect_number_7800, orig_redirect_number, term_redirect_number, capacity " \
            "   from auth.trunk " \
