...
protected:
    ...
    string sql(BDb * db) {
        return "   select id, voip_limit_day, credit, balance, amount_date, voip_disabled, organization_id, price_include_vat, timezone_offset, is_blocked, anti_fraud_disabled, account_version " \
    ...
    inline void parse_item(BDbResult &row, Client * item) {
        ...
        item->anti_fraud_disabled = row.get_b(10);
        item->account_version = row.get_i(11);
    }
