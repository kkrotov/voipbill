...
void ThreadSyncCalls::getCentalId(string month, long long int &central_id, string &central_time) {
    ...
    auto res2 = db_main.query("select id, connect_time from calls_raw.calls_raw_" + month + " where server_id in " +
...
bool ThreadSyncCalls::copyCallsPart(string month, unsigned long limit) {
    ...
    try {
        ...
        BDb::copy("calls_raw.calls_raw_" + suffix,
                ...
                " nnp_number_range_id, nnp_operator_id, nnp_region_id, nnp_city_id, nnp_country_prefix, nnp_ndc, nnp_is_mob, nnp_country_code,"\
                " is_include_vat, nnp_package_minute_id, nnp_package_price_id, nnp_package_pricelist_id, signalling_call_id, hash, "
