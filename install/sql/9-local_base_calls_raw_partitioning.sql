...
DROP TRIGGER partitioning ON calls_raw.calls_raw;
DROP FUNCTION calls_raw.tr_partitioning();
DROP FUNCTION calls_raw.create_calls_raw_partition(timestamp without time zone);
...
BEGIN
	...
	THEN
		...
		EXECUTE 'ALTER TABLE ' || relname || ' OWNER TO g_bill_daemon_local';
...
CREATE TRIGGER partitioning
  BEFORE INSERT
  ON calls_raw.calls_raw
  FOR EACH ROW
  EXECUTE PROCEDURE calls_raw.tr_partitioning();
