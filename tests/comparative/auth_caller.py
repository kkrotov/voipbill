...
print 'Auth caller - запускает тест calcAuth из auth.test_auth'
print 'Результат записывает в tests.auth_tests'
...
centralDbCur.execute('''
  ...
  CREATE TABLE IF NOT EXISTS tests.auth_tests (
  id      SERIAL,          -- id теста
...
def testRegion(region):
    for id, server_id, trunk_name, src_number, dst_number, src_noa, dst_noa, redirect_number in testRows:
        ...
        resultQuery = 'INSERT INTO tests.auth_tests (id, date, answer) VALUES ('
        resultQuery += str(id)
        resultQuery += ', \''
