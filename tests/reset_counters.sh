...
/usr/pgsql-9.4/bin/psql -U postgres nispd_test -c "delete from billing.stats_trunk_settings; delete from billing.stats_package;"


for region_id in `echo $REGIONS_LIST`
do
  echo "Очищаем счетчики в в региональной БД $region_id"
  /usr/pgsql-9.4/bin/psql -U postgres nispd${region_id}_test -c "delete from billing.stats_trunk_settings; delete from billing.stats_package;"
done
