...
logging.info('Start syncstat script')


class Sync(Daemon):
    def init(self):
        self.db_stat = MySQLdb.connect(host=config['stat']['host'], user=config['stat']['user'],
                                       passwd=config['stat']['password'], db=config['stat']['db'])
        self.db_stat.autocommit(True)

        self.db = psycopg2.connect(host=config['bill']['host'], user=config['bill']['user'],
                                   password=config['bill']['password'], database=config['bill']['db'])
    ...
    def do_sync(self):
        ...
        while need:
            ...
            if n > 0:
                self.log_oprations(n, 'organization')
            ...
            if n > 0:
                self.log_oprations(n, 'usage_voip')
                # print '+', n, 'usage_voip'
                if n == 100:
                    need = True
            ...
            if n > 0:
                self.log_oprations(n, 'usage_voip_package')
            ...
            if n > 0:
                self.log_oprations(n, 'usage_trunk')
                # print '+', n, 'usage_trunk'
                if n == 100:
                    need = True
            ...
            if n > 0:
                self.log_oprations(n, 'usage_trunk_settings')
            ...
            if n > 0:
                self.log_oprations(n, 'log_tarif')
                # print '+', n, 'log_tarif'
                if n == 100:
                    need = True
            ...
            if n > 0:
                self.log_oprations(n, 'prefixlist')
                # print '+', n, 'prefixlist'
                if n == 100:
                    need = True
            ...
            if n > 0:
                self.log_oprations(n, 'destination_prefixlists')
            ...
            if n > 0:
                self.log_oprations(n, 'tariff_package')
            ...
            if n > 0:
                self.log_oprations(n, 'currency_rate')
    ...
    def do_sync_clients(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            left join
                              ...
                              on
                                cc.id=c.contract_id
                            where
                              z.tbase='""" + tbase + """' and z.tname='clients'
                            limit
                              """ + str(partsize))
        todel = []
        toins = []
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.clients(id, voip_limit_month, voip_limit_day, voip_limit_mn_day, voip_disabled, balance, credit,amount_date,last_payed_month,organization_id,price_include_vat,is_blocked,timezone_offset,anti_fraud_disabled,account_version,currency)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_organization(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            left join organization o
                              on
                                z.tid=o.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='organization'
                            limit
                              """ + str(partsize))
        todel = []
        toins = []
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.organization(id, organization_id, actual_from, actual_to, tax_rate)VALUES(%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_usage_voip(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            from
                              z_sync_postgres z
                            left join usage_voip c
                              on
                                z.tid=c.id
                            left join clients cc
                              on
                                c.client=cc.client
                            left join voip_numbers vn
                              on
                                vn.number = c.E164
                            where
                              z.tbase='""" + tbase + """' and z.tname='usage_voip'
                                UNION
				            select
				              z.rnd,
				              z.tid,
				            ...
				            left join
				              clients cc
				              on
				                at.client_account_id=cc.id
				            left join
				              voip_numbers vn
				              on
				                vn.uu_account_tariff_id = at.id
				            left join
				              city
				                on
				                  at.city_id = city.id
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.service_number(id, client_account_id, activation_dt, expire_dt, did, lines_count, server_id, tech_number, tech_number_operator_id)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_usage_voip_package(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            from
                              z_sync_postgres z
                            left join
                              usage_voip_package c
                              on
                                z.tid=c.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='usage_voip_package'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.service_package(id, activation_dt, expire_dt, tariff_package_id, service_number_id, service_trunk_id)VALUES(%s,%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_usage_trunk(self, partsize):
        ...
        cur_stat.execute("""select
							...
							left join
							  clients cl
							  on
							    c.client_account_id = cl.id
							left join
							  client_contract cc
							  on
							    cc.id = cl.contract_id
                            where
                              z.tbase='""" + tbase + """' and z.tname='usage_trunk'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
    ...
    def do_sync_usage_trunk_settings(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            from
                              z_sync_postgres z
                            left join
                              usage_trunk_settings c
                              on
                                z.tid=c.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='usage_trunk_settings'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
    ...
    def do_sync_tarif(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            left join
                              ...
                              on
                                z.tid=c.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='tarifs_voip'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.tariff(id, name, currency_id, status,freemin,freemin_for_number,pricelist_id,paid_redirect,tariffication_by_minutes,tariffication_full_first_minute,tariffication_free_first_seconds)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_log_tarif(self, partsize):
        ...
        cur_stat.execute("""select
                              ...
                              c.date_activation,
                              c.id_tarif,
                              c.id_tarif_local_mob,
                              c.id_tarif_russia,
                              c.id_tarif_russia_mob,
                              c.id_tarif_intern,
                              c.id_tarif_sng
                            from
                              z_sync_postgres z
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.tariff_change_log (id, usage_id, ts, date_activation, tarif_id_local, tarif_id_local_mob, tarif_id_russia, tarif_id_russia_mob, tarif_id_intern, tarif_id_sng)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_prefixlist(self, partsize):
        ...
        cur_stat.execute("""select
                              ...
                              region_id,
                              city_id,
                              case exclude_operators when true then 't' else 'f' end, operators
                            from
                              z_sync_postgres z
                            left join
                              voip_prefixlist c
                              on
                                z.tid=c.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='prefixlist'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.stat_prefixlist(id, type_id, sub_type, prefixes, country_id, region_id, city_id, exclude_operators, operators)VALUES(%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_destination_prefixlists(self, partsize):
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.stat_destination_prefixlists(id, destination_id, prefixlist_id)VALUES(%s,%s,%s)",
                toins)
        if len(todel) > 0:
    ...
    def do_sync_tariff_package(self, partsize):
        ...
        cur_stat.execute("""select
                            ...
                            from
                              z_sync_postgres z
                            left join
                              tarifs_voip_package c
                              on
                                z.tid=c.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='tariff_package'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
        ...
        if len(toins) > 0:
            cur.executemany(
                "INSERT INTO billing.tariff_package(id, destination_id, prepaid_minutes, pricelist_id)VALUES(%s,%s,%s,%s)",
                toins)
        if len(todel) > 0:
        ...
        cur_stat.execute("""select
                            ...
                            left join currency_rate c
                              on
                                z.tid=c.id
                            where
                              z.tbase='""" + tbase + """' and z.tname='currency_rate'
                            limit """ + str(partsize))
        todel = []
        toins = []
        tofix = []
    ...
    def run(self):
        while True:
            try:
                self.init()
                while True:
                    # logging.info('Start check a new data')
